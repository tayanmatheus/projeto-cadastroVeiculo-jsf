...
public class VeiculoDAO {
	...
	public void incluir(Veiculo veiculo) throws ClassNotFoundException, SQLException {

		Conexao conexao = new Conexao();
		Connection conn = conexao.conectarNoBancoDeDados();
		String sql = "INSERT INTO veiculos "
				+ "(marca_veiculo, fabricante_veiculo, valor_veiculo, data_fabricacao_veiculo) " + "VALUES (?,?,?,?)";
	...
	public void alterar(Veiculo veiculo) throws ClassNotFoundException, SQLException {
		...
		Conexao conexao = new Conexao();
		Connection conn = conexao.conectarNoBancoDeDados();
		String sql = "UPDATE veiculos SET marca_veiculo = ?," + " fabricante_veiculo = ?," + " valor_veiculo = ?,"
				+ " data_fabricacao_veiculo = ?" + "WHERE id_veiculo = ?";
		PreparedStatement ps = conn.prepareStatement(sql);
		ps.setString(1, veiculo.getMarca().trim());
		ps.setString(2, veiculo.getFabricante().trim());
		ps.setDouble(3, veiculo.getValor());
	...
	public List<Veiculo> Listar() throws ClassNotFoundException, SQLException {
		...
		Conexao conexao = new Conexao();
		Connection conn = conexao.conectarNoBancoDeDados();
		String sql = "SELECT id_veiculo," + " marca_veiculo," + " fabricante_veiculo," + " valor_veiculo,"
				+ " data_fabricacao_veiculo" + " FROM veiculos";
		PreparedStatement ps = conn.prepareStatement(sql);
